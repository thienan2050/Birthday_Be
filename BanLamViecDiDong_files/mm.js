...
((window, navigator) => {
  ...
  const config = {
    debug: true,
    endpoint: 'https://a.mrktmtrcs.net',
  ...
  function debugMessage(level, message) {
    ...
    console.debug(`%c [MM-${level}-${elapsed}] ${message}`, 'background: #222222; color: #a8d5e5');
  }
  ...
  function preprocess() {
    ...
    const urlDebugMatches = window.location.search.match(/[?&]mmdebug=(true|1)/);
    if (urlDebugMatches) {
      config.debug = true;
